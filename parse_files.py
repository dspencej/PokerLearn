...
def process_player_action_line(line, round_entry, game_number, hand):
    try:
        action_keywords = ['re-raises', 'posts', 'calls', 'raises', 'folds']
        ...
        if player_record:
            player_action = PlayerAction(round_id=round_entry.id, player_id=player_record.id, action=action, amount=amount)
            db.session.add(player_action)
            logger.debug(f"Added action {action} by player {player_name} for {amount} chips in round {round_entry.round_name} for hand {hand.hand_number} in game {game_number}")
        else:
            logger.error(f"Player record not found for {player_name} when processing action line: {line}")
    except Exception as e:
...
def process_show_action_line(line, round_entry, game_number, hand):
    try:
        ...
        if player_record:
            player_action = PlayerAction(round_id=round_entry.id, player_id=player_record.id, action=action, amount=amount)
            db.session.add(player_action)
            logger.debug(f"Player {player_name} shows cards in round {round_entry.round_name} for hand {hand.hand_number} in game {game_number}")
        else:
            logger.error(f"Player record not found for {player_name} when processing show line: {line}")
    except Exception as e:
...
def process_seat_line(line, hand, game_number, round_entry):
    ...
    if not player_record:
        ...
        db.session.commit()
        logger.debug(f"Created new player entry {player} at seat {seat} with {chips} chips in hand {hand.hand_number} for game {game_number}")
    player_action = PlayerAction(round_id=round_entry.id, player_id=player_record.id, action='seat', amount=chips)
    db.session.add(player_action)
    logger.debug(f"Added seat action for player {player} with {chips} chips in round {round_entry.round_name} for hand {hand.hand_number} in game {game_number}")
...
def process_dealing_line(line, round_entry, game_number, hand):
    ...
    logger.debug(f"Dealt card {card} for round {round_entry.round_name} in hand {hand.hand_number} for game {game_number}")
...
def process_player_leaves_line(line):
    ...
    player_record = Player.query.filter_by(name=player_name).first()
    if player_record:
        logger.debug(f"Player {player_name} leaves the table with {player_record.chips_start} chips")
    else:
        logger.error(f"Player record not found for {player_name} when processing leave line: {line}")
    ...
    for line in lines:
        ...
        try:
            if line.startswith('Game #') and 'starts' in line:
                ...
                round_number += 1
                round_entry = create_new_round(hand.id, round_number)
                logger.debug(f"Round {round_number} is over, created new round entry for hand {hand.hand_number} in game {game_number}")
            ...
            elif hand_started and any(action in line for action in ['posts', 'calls', 'raises', 're-raises', 'folds']):
        ...
        for file in tqdm(files, desc="Processing Files"):
            ...
            with open(os.path.join(data_folder, file), 'r') as f:
                ...
                if not game:
                    ...
                    db.session.add(game)
                    db.session.commit()
                    logger.debug(f"Created new game entry: {game_number} with date: {game_date} and time: {game_time}")
                else:
                    logger.debug(f"Game {game_number} already exists in the database. Processing hands...")
